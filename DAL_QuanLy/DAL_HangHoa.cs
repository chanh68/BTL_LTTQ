...
namespace DAL_QuanLy
{
    public class DAL_HangHoa : DBConnect
    {
        ...
        public List<DTO_HangHoa> LayDanhSachHangHoaTheoNCC(string maNCC)
        {
            ...
            string query = "SELECT MaHang, TenHang, DonGiaBan FROM HangHoa WHERE TrangThai = 1 AND MaNCC = @MaNCC";
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            DTO_HangHoa hangHoa = new DTO_HangHoa
                            {
                                MaHang = reader["MaHang"]?.ToString(),
                                TenHangHoa = reader["TenHang"]?.ToString(),
                                DonGiaBan = reader["DonGiaBan"] != DBNull.Value ? Convert.ToDecimal(reader["DonGiaBan"]) : 0
                            };
            ...
            {
                // Ghi log lỗi hoặc xử lý theo nhu cầu
                Console.WriteLine("Lỗi khi lấy danh sách hàng hóa theo nhà cung cấp: " + ex.Message);
            }
            finally
            {
                CloseConnection();
